...
local function abrirMenuCheats()
    ...
    botaoFlingAll.MouseButton1Click:Connect(function()
        ...
        flingAllActive = true
        botaoFlingAll.Text = "Fling All: ON"
        botaoFlingAll.BackgroundColor3 = Color3.fromRGB(0,255,0)

        local myHRP = myChar.HumanoidRootPart
        local wasNoclip = noclipActive
        local wasFly = flyActive

        noclipToggle(true)

        flingAllThread = coroutine.create(function()
            local playersList = {}
            ...
            flingForce:Destroy()
            if not wasNoclip then noclipToggle(false) end
            if wasFly and not flyActive then startFly() end
